# app.py — OASIS Longitudinal (deterministic, NaN-safe, Converted->Demented, ranges+warnings)
...
def make_pipeline(num_cols: List[str], cat_cols: List[str], model_name: str, params: Dict):
    # Robust preprocessing: imputation + scaling/OHE (kept consistent for reproducibility)
    ...
    if model_name == "Logistic Regression":
        clf = LogisticRegression(max_iter=params.get("max_iter", 300), C=params.get("C", 1.0), random_state=42)
        ...
        clf = GradientBoostingClassifier(
            n_estimators=params.get("n_estimators", 300),   # spelling fixed
...
# ---- Clean missing/infinite values AFTER selecting target ----
...
if page == "Overview":
    ...
    if "trained_model" not in st.session_state or "feature_columns" not in st.session_state:
        ...
        if predict_btn:
            # Assemble row for prediction (unchanged by UI hints)
            x_row = pd.DataFrame([inputs])[feat_cols]

            # Gentle validation: flag out-of-range numeric inputs (does not modify values)
            warnings = []
            ...
            # Predict (identical behavior; no clamping)
